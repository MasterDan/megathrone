...
export const ProfileEndpoints: Component = () => {
  const params = useParams();
  const location = useLocation();
  const navigate = useNavigate();
  const profileId = createMemo(() => Number(params.id));
  ...
  const selectionPlaceholder = () =>
    activeMode() === "manual" ? "Pick an endpoint below" : "Choosing automatically…";
        ...
        {(data) => (
          <>
            <div class="sticky top-14 z-40 -mx-4 rounded-2xl border border-base-content/10 bg-base-100/60 px-4 py-2.5 shadow-sm backdrop-blur-md">
              {/* Row 1: identity — live selection — edit. The selection chip
                  fills the space right of the title. */}
              <div class="flex items-center gap-2">
                ...
                <div class="min-w-0">
                  <h1 class="truncate text-lg font-bold leading-tight" title={data().name}>
                    {data().name}
                  </h1>
                  <p
                    class="truncate text-xs text-base-content/50"
                    title={data().sourceUrl ?? data().sourcePath ?? undefined}
                  >
                    {sourceLabel(data())}
                  </p>
                </div>
                <div class="ml-auto flex min-w-0 items-center gap-1.5">
                  ...
                  >
                    {(item) => (
                      <>
                        {/* The chip is also a jump link: clicking it scrolls
                            the grid to the selected card. */}
                        <button
                          type="button"
                          ...
                          onClick={() => itemsApi?.revealItem(item().id)}
                        >
                          <span
              ...
              {/* Row 2: strategy — profile stats. The stats sit inline on wide
                  screens; below `lg` they collapse into the info modal. */}
              <div class="mt-2 flex items-center gap-2">
                <OptionTabs
                  class="min-w-0 flex-1"
                  items={SELECTION_TABS}
                  value={activeMode()}
                  onChange={(mode) => void selection.setMode(mode)}
                />
                <div class="hidden shrink-0 items-center gap-1.5 whitespace-nowrap text-xs text-base-content/50 lg:flex">
                  <span title="Endpoints in this profile">{data().itemCount} endpoints</span>
                  <Show when={data().skippedCount > 0}>
                    <span aria-hidden="true">·</span>
                    <span title="Lines that could not be parsed">
                      {data().skippedCount} skipped
                    </span>
                  </Show>
                  <Show when={data().autoUpdateMinutes}>
                    {(minutes) => (
                      <>
                        <span aria-hidden="true">·</span>
                  ...
                  <span aria-hidden="true">·</span>
                  <span title={`Fetched ${formatRelativeTime(data().lastFetchedAt)}`}>
                    updated {formatRelativeTime(data().lastFetchedAt)}
                  </span>
                </div>
                <button
                  type="button"
            ...
            {/* Narrow screens only (the ⓘ button): the stats that sit inline
