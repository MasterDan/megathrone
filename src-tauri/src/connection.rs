    ...
    if mode == MODE_TUN && !running_as_root() {
        return Err(
            "TUN mode needs administrator privileges to create the network interface, \
             but the app is running as a regular user. Launch the app elevated \
             (on macOS/Linux build it and run the binary with sudo, or `sudo pnpm \
             tauri dev` for a quick dev loop; on Windows run it as administrator) — \
             or use System Proxy mode instead."
                .to_string(),
        );
    }
    ...
    if mode == MODE_TUN && denied {
        format!(
            "{reason}\n\nTUN mode needs administrator privileges to create the network \
             interface. Launch the app elevated (on macOS/Linux run it with sudo, on \
             Windows as administrator) — or use System Proxy mode instead."
        )
    } else {
        reason.to_string()
    }
